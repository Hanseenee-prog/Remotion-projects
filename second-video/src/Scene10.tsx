// Scene 10 — "Watch my last video on debounce. Follow for more!"
// 120 frames — identical structure to debounce Scene9
...
const ProfileCard: React.FC<{
  ...
  rippleScale: number; rippleOpacity: number; showCursor: boolean;
}> = ({
  ...
  return (
    <div style={{
      ...
      transformOrigin: "center center", zIndex: 50,
    }}>
      <div style={{
...
export const Scene10: React.FC = () => {
  ...
  const teaserOpacity = interpolate(frame, [0, 8, TEASER_OUT, TEASER_OUT + 8], [0, 1, 1, 0]);
  const teaserBlur    = interpolate(teaserSpring, [0, 1], [8, 0]);

  // Card
  const cardInSpring = spring({ fps, frame: frame - CARD_START, config: { damping: 14, stiffness: 130, mass: 0.9 } });
  const cardScale    = interpolate(clamp(cardInSpring), [0, 1], [0.7, 1]);
  const cardOpacity  = interpolate(frame, [CARD_START, CARD_START + 10], [0, 1], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });

  // Cursor
  const cursorMoveSpring = spring({ fps, frame: frame - CURSOR_IN, config: { damping: 14, stiffness: 100 } });
  ...
  const cursorFade    = interpolate(frame, [CLICK_AT, CLICK_AT + 8], [1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  ...
  return (
    <AbsoluteFill style={{
      ...
      <div style={{
        ...
        <div style={{
          fontFamily: FONTS.mono, fontSize: 30, color: COLORS.accentC,
          fontWeight: 700, letterSpacing: 2, marginBottom: 18,
        }}>
          WATCH NEXT
        </div>
        <div style={{
          fontFamily: FONTS.display, fontSize: 58, color: "white",
          fontWeight: 900, maxWidth: 800, lineHeight: 1.2,
        }}>
          Learn about{" "}
          <span style={{ color: COLORS.accentB }}>Debounce</span>
          {" "}in my last video
        </div>
      </div>
